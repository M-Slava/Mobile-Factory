...
function Event.somethingWasPlaced(event)
    ...
    if entity == nil or entity.last_user == nil then
        return
    end
    ...
    -- Check the Values --
    if entity == nil or MFPlayer == nil or playerMF == nil then
        return
    end
    ...
    if type ~= "entity-ghost" and entity.name == "InternalEnergyCube" then
        ...
        if MF.internalEnergyObj.ent ~= nil and MF.internalEnergyObj.ent.valid == true then
            if event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true then
                MFPlayer.ent.print({"", {"gui-description.MaxPlaced"}, " ", {"item-name." .. event.consumed_items[1].name}})
            end
            destroyEntity = true
        elseif destroyEntity == false then
            MF.internalEnergyObj:setEnt(entity)
            if event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true then
                local tags = event.consumed_items[1].get_tag("Infos")
                if tags ~= nil then
    ...
    if destroyEntity == true then
        entity.destroy()
        if event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true then
            MFPlayer.ent.get_main_inventory().insert(event.consumed_items[1])
        end
        return
    end
    ...
    if _mfTagsTable[entity.name] ~= nil then
        ...
        if event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true and event.consumed_items[1].type == "item-with-tags" and
            objTable.itemTagsToContent ~= nil then
    ...
    if objInfo ~= nil and objInfo.noPlaced ~= true and objInfo.tag ~= nil then
        ...
        -- Check if there are Item Tags --
        if event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true and event.consumed_items[1].type == "item-with-tags" then
            local tags = event.consumed_items[1].get_tag("Infos")
            if tags ~= nil then
...
function Event.somethingWasCloned(event)
    ...
    if event.source ~= nil and event.source.valid == true and event.destination ~= nil and event.destination.valid ==
        true and string.match(event.source.name, "MobileFactory") then
...
function Event.somethingWasRemoved(event)
    ...
    if removedEnt == nil or removedEnt.valid == false then
        return
    end
...
function tilesWasPlaced(event)
    ...
    if event.tiles ~= nil and MFFloor == _mfControlSurfaceName then
        ...
        if MFPlayer ~= nil and event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true then
            MFPlayer.ent.print({"", {"item-name." .. event.consumed_items[1].name}, " ", {"gui-description.CCNotPlaceable"}})
        end
        return
    end
...
function placedMobileFactory(event, entity, MFPlayer, MF)
    ...
    if entity.valid == false and event.consumed_items ~= nil and event.consumed_items[1].valid_for_read == true then
        MFPlayer.ent.get_main_inventory().insert(event.consumed_items[1])
    end
end
...
function placedDeploy(ent, MFPlayer, mf)
    ...
    local ents = ent.surface.find_entities_filtered {
        type = "car",
        position = ent.position,
        radius = 7
    }
    if ents[1] ~= nil and ents[1].valid == true and ents[1].last_user ~= nil and
        string.match(ents[1].name, "MobileFactory") and getMF(ents[1].last_user.name) ~= nil and
        getMF(ents[1].last_user.name) == mf then
        -- Check if the Mobile Factory is not Moving --
        if ents[1].speed ~= 0 then
            MFPlayer.ent.create_local_flying_text {
                text = {"gui-description.DPGUIMFMoving"},
                position = ent.position
            }
            MFPlayer.ent.cursor_stack.set_stack({
                name = "MFDeploy",
                count = 1
            })
        else
        ...
        MFPlayer.ent.create_local_flying_text {
            text = {"gui-description.DPGUINoMF", MF.name},
            position = ent.position
        }
        MFPlayer.ent.cursor_stack.set_stack({
            name = "MFDeploy",
...
function checkCCTile(entity)
    local tile = entity.surface.find_tiles_filtered {
