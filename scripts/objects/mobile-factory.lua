...
function MF:construct(object)
    ...
    self.ent = object
    if self.fS == nil or self.fS.valid == false then
        self.fS = nil
        createMFSurface(self)
    end
    if self.ccS == nil or self.ccS.valid == false then
        self.ccS = nil
        createControlRoom(self)
    end
...
function MF:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    if justCreated == true then
        ...
        GAPI.addSimpleButton(GUITable, "M.F.OpenInvButton", inventoryFlow, {"gui-description.OpenInventory"}, "", false,
            {
                ID = self.player
            })
        ...
        if Util.technologyUnlocked("EnergyDrain1", getForce(self.player)) then
            GAPI.addLabel(GUITable, "", laserFrame, {"gui-description.MFEnergyLaser"}, nil, "", false, nil,
                _mfLabelType.yellowTitle)
            local state = "left"
            if self.selectedEnergyLaserMode == "output" then
                state = "right"
            end
            GAPI.addSwitch(GUITable, "M.F.EnergyLasersSwitch", laserFrame, {"gui-description.Drain"},
                {"gui-description.Send"}, {"gui-description.MFDrainTT"}, {"gui-description.MFSendTT"}, state, false, {
                    ID = self.playerIndex
                })
        end
        ...
        if Util.technologyUnlocked("EnergyDrain1", getForce(self.player)) then
            GAPI.addLabel(GUITable, "", laserFrame, {"gui-description.MFQuatronLaser"}, nil, "", false, nil,
                _mfLabelType.yellowTitle)
            local state = "left"
            if self.selectedQuatronLaserMode == "output" then
                state = "right"
            end
            GAPI.addSwitch(GUITable, "M.F.QuatronLasersSwitch", laserFrame, {"gui-description.Drain"},
                {"gui-description.Send"}, {"gui-description.MFDrainTT"}, {"gui-description.MFSendTT"}, state, false, {
                    ID = self.playerIndex
                })
        end
        ...
        if Util.technologyUnlocked("FluidDrain1", getForce(self.player)) then
            -- Send/Drain --
            GAPI.addLabel(GUITable, "", laserFrame, {"gui-description.MFFluidLaser"}, nil, "", false, nil,
                _mfLabelType.yellowTitle)
            local state = "left"
            if self.selectedFluidLaserMode == "output" then
                state = "right"
            end
            GAPI.addSwitch(GUITable, "M.F.FluidLasersSwitch", laserFrame, {"gui-description.Drain"},
                {"gui-description.Send"}, {"gui-description.MFDrainTT"}, {"gui-description.MFSendTT"}, state, false, {
                    ID = self.playerIndex
                })
            -- Target --
            ...
            GAPI.addDropDown(GUITable, "M.F.FluidLasersTargetDD", laserFrame, invs, selectedIndex, false,
                {"gui-description.MFFluidLasersTargetTT"}, {
                    ID = self.playerIndex
                })
        end
...
-- Return the Shield --
function MF:shield()
    if self.ent == nil or self.ent.valid == false or self.ent.grid == nil then
        return 0
    end
...
function MF:scanEnt()
    ...
    if self.energyLaserActivated and Util.technologyUnlocked("EnergyDrain1", selfForce) then
        self.ELEntities = selfSurface.find_entities_filtered {
            position = selfPosition,
            force = selfForce,
            radius = selfRadius,
            name = _mfEnergyLaserTargets
        }
    else
        self.ELEntities = nil
    end
    ...
    if self.quatronLaserActivated and Util.technologyUnlocked("EnergyDrain1", selfForce) and
        Util.technologyUnlocked("QuatronLogistic", selfForce) then
        self.QLEntities = selfSurface.find_entities_filtered {
            position = selfPosition,
            force = selfForce,
            radius = selfRadius,
            name = _mfQuatronLaserTargets
        }
    else
        self.QLEntities = nil
    end
    ...
    if self.fluidLaserActivated and Util.technologyUnlocked("FluidDrain1", selfForce) then
        self.FLEntities = selfSurface.find_entities_filtered {
            position = selfPosition,
            force = selfForce,
            radius = selfRadius,
            type = "storage-tank"
    ...
    if self.itemLaserActivated and Util.technologyUnlocked("TechItemDrain", selfForce) then
        self.ILEntities = selfSurface.find_entities_filtered {
            position = selfPosition,
            force = selfForce,
            radius = selfRadius,
...
function MF:updateQuatronLaser(entity)
    ...
    if obj == nil or obj.ent == nil or obj.ent.valid == false then
        return
    end
    ----------------------- Drain Quatron -------------------------
    if self.selectedQuatronLaserMode == "input" and self.internalQuatronObj.ent ~= nil and
        self.internalQuatronObj.ent.valid == true and EI.energy(obj) > 0 then
        -- Missing Internal Quatron or Structure Quatron or LaserDrain Caparity --
        local drainedQuatron = math.min(EI.maxEnergy(self.internalQuatronObj) - EI.energy(self.internalQuatronObj),
            EI.energy(obj), self:getLaserQuatronDrain(), EI.speed(obj))
        ...
        if drainedQuatron > 0 then
            ...
            -- Create the Beam --
            self.ent.surface.create_entity {
                name = "PurpleQuatronBeam",
                duration = 60,
                position = self.ent.position,
                target_position = entity.position,
                source_position = {self.ent.position.x, self.ent.position.y}
            }
            -- One less Beam to the Beam capacity --
            return true
        end
        ----------------------- Send Quatron -------------------------
    elseif self.selectedQuatronLaserMode == "output" and self.internalQuatronObj.ent ~= nil and
        self.internalQuatronObj.ent.valid == true and EI.energy(obj) < EI.maxEnergy(obj) then
        -- Structure missing Quatron or Laser Power or Mobile Factory Quatron --
        local quatronSend = math.min(EI.maxEnergy(obj) - EI.energy(obj), self:getLaserQuatronDrain(),
            EI.energy(self.internalQuatronObj), EI.speed(obj))
...
function MF:updateEnergyLaser(entity)
    ...
    ----------------------- Drain Energy -------------------------
    if self.selectedEnergyLaserMode == "input" and self.internalEnergyObj.ent ~= nil and
        self.internalEnergyObj.ent.valid == true and entity.energy > 0 then
        -- Missing Internal Energy or Structure Energy or LaserDrain Caparity --
        local drainedEnergy = math.min(EI.maxEnergy(self.internalEnergyObj) - EI.energy(self.internalEnergyObj),
            entity.energy, self:getLaserEnergyDrain(), EI.speed(obj))
        ...
        ----------------------- Send Energy -------------------------
    elseif self.selectedEnergyLaserMode == "output" and self.internalEnergyObj.ent ~= nil and
        self.internalEnergyObj.ent.valid == true and entity.energy < entity.electric_buffer_size then
        ...
        local energySend = math.min(entity.electric_buffer_size - entity.energy, self:getLaserEnergyDrain(),
            EI.energy(self.internalEnergyObj), EI.speed(obj))
...
function MF:updateFluidLaser(entity)
    ...
    if self.selectedFluidLaserMode == "input" then
        if localFluid == nil then
            return
        end
        -- Check the local and distant Tank --
        if distantTank:canAccept(localFluid) == false then
            return
        end
        -- Send the Fluid --
        local amountAdded = distantTank:addFluid(localFluid)
        -- Remove the local Fluid --
        localTank.remove_fluid({
            name = localFluid.name,
            amount = amountAdded,
            minimum_temperature = -300,
            maximum_temperature = 1e7
        })
        if amountAdded > 0 then
            ...
            self.ent.surface.create_entity {
                ...
                duration = 60,
                position = self.ent.position,
                target = localTank.position,
                source = self.ent.position
            }
            -- Drain Energy --
    ...
    elseif self.selectedFluidLaserMode == "output" then
        -- Check the local and distant Tank --
        if localFluid and localFluid.name ~= distantTank.inventoryFluid then
            return
        end
        if distantTank.inventoryFluid == nil or distantTank.inventoryCount == 0 then
            return
        end
        -- Get the Fluid --
        local amountAdded = localTank.insert_fluid({
            name = distantTank.inventoryFluid,
            amount = distantTank.inventoryCount,
            temperature = distantTank.inventoryTemperature
        })
        -- Remove the distant Fluid --
        distantTank:getFluid({
            name = distantTank.inventoryFluid,
            amount = amountAdded
        })
        if amountAdded > 0 then
...
function MF:updateLogisticLaser(entity)
    ...
    if EI.energy(self.internalEnergyObj) > _mfBaseItemEnergyConsumption * self:getLaserItemDrain() then
        ...
        if inv ~= nil and inv.valid == true then
            ...
            local capItems = math.min(self:getLaserItemDrain(),
                EI.energy(self.internalEnergyObj) * _mfBaseItemEnergyConsumption)
...
function MF:updateShield(event)
    ...
    if self:shield() > 0 then
        ...
        rendering.draw_animation {
            animation = "mfShield",
            target = {self.ent.position.x - 0.25, self.ent.position.y - 0.3},
            tint = {1, tint, tint},
            time_to_live = 2,
            x_scale = size,
            y_scale = size,
...
function MF:factoryTeleportBox()
    ...
    if self.tpEnabled == true then
        ...
        local entities = self.ent.surface.find_entities_filtered {
            area = {{mfB.left_top.x - 0.5, mfB.left_top.y - 0.5}, {mfB.right_bottom.x + 0.5, mfB.right_bottom.y + 0.5}},
            type = "character"
        }
    ...
    -- Factory to Outside --
    if self.fS ~= nil then
        local entities = self.fS.find_entities_filtered {
            area = {{-1, -1}, {1, 1}},
            type = "character"
        }
    ...
    if Util.technologyUnlocked("ControlCenter", getForce(self.player)) ~= false and self.ccS ~= nil and self.fS ~= nil then
        local entities = self.ccS.find_entities_filtered {
            area = {{-3, 5}, {3, 8}},
            type = "character"
        }
        for k, entity in pairs(entities) do
...
function MF:scanModules()
    ...
    if Util.technologyUnlocked("EnergyPowerModule", getForce(self.player)) == nil then
        return
    end
    -- Init Variables --
...
function MF:TPMobileFactoryPart1(location)
    ...
    local tpCoords = location.surface.find_non_colliding_position(self.MF.ent.name, {location.posX, location.posY}, 10,
        0.1, false)
    ...
    local animation1 = rendering.draw_animation {
        animation = "SimpleTPAn",
        animation_speed = 0.5,
        render_layer = 131,
        x_scale = 4,
        y_scale = 3.5,
        target = {self.ent.position.x, self.ent.position.y - 0.7},
        surface = self.ent.surface,
        time_to_live = 150 * 2
    }
    Util.resetAnimation(animation1, 150)
    local animation2 = rendering.draw_animation {
        animation = "SimpleTPAn",
        animation_speed = 0.5,
        render_layer = 131,
        x_scale = 4,
        y_scale = 3.5,
        target = {location.posX, location.posY - 0.7},
        surface = location.surface,
        time_to_live = 150 * 2
    }
    Util.resetAnimation(animation2, 150)
    -- Play all Sounds --
    self.ent.surface.play_sound {
        path = "MFSimpleTP",
        position = self.ent.position
    }
    self.ent.surface.play_sound {
    ...
    for k, slot in pairs(self.slots) do
        ...
        if outEnt ~= nil and outEnt.valid == true and inEnt ~= nil and inEnt.valid == true then
            ...
            if string.match(slot.entity, "Pole") then
                local ocWC = outEnt.get_wire_connector(defines.wire_connector_id.pole_copper, true)
                local orWC = outEnt.get_wire_connector(defines.wire_connector_id.circuit_red, true)
                local ogWC = outEnt.get_wire_connector(defines.wire_connector_id.circuit_green, true)

                local icWC = inEnt.get_wire_connector(defines.wire_connector_id.pole_copper, true)
                local irWC = inEnt.get_wire_connector(defines.wire_connector_id.circuit_red, true)
                local igWC = inEnt.get_wire_connector(defines.wire_connector_id.circuit_green, true)
