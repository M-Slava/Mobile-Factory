...
function DSR:contentToItemTags(tags)
    if self.inventoryItem ~= nil or self.filter ~= nil then
        ...
        tags.custom_description = {"", tags.prototype.localised_description,
                                   {"item-description.DeepStorageC", self.inventoryItem or self.filter,
                                    self.inventoryCount or 0}}
    end
end
...
function DSR:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    if justCreated == true then
        ...
        GAPI.addLabel(GUITable, "", settingsFrame, {"gui-description.DSDTMaxItemsAmount"}, nil,
            {"gui-description.DSDTMaxItemsAmountTT"}, false, nil, _mfLabelType.yellowTitle)
        local textfield = GAPI.addTextField(GUITable, "D.S.R.Max", settingsFrame, self.max, "", false, true, false,
            false, false, {
                ID = self.ent.unit_number
            })
