...
function EC:contentToItemTags(tags)
    if self.ent.energy > 0 then
        ...
        tags.custom_description = {"", tags.prototype.localised_description,
                                   {"item-description.EnergyCubeC", Util.toRNumber(math.floor(self.ent.energy))}, "J"}
    end
end
...
function EC:update()
    ...
    -- Update the Sprite --
    local spriteNumber = math.ceil(
        self.ent.energy / self.ent.prototype.electric_energy_source_prototype.buffer_capacity * 16)
    if (self.sprite) then
        self.sprite.destroy()
    end
