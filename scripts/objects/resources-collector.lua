...
function RCL.new(ent)
    ...
    -- Create the Tank --
    t.tank = ent.surface.create_entity {
        name = "ResourcesCollectorTank",
...
function RCL.remove(obj)
    ...
    -- Destroy the Tank --
    if obj.tank ~= nil and obj.tank.valid == true then
        obj.tank.destroy()
    end
...
function RCL.contentToItemTags(obj, tags)
    if EI.energy(obj) > 0 then
        tags.set_tag("Infos", {
            charge = EI.energy(obj),
            purity = EI.energyLevel(obj)
        })
        tags.custom_description = {"", tags.prototype.localised_description,
                                   {"item-description.ResourcesCollectorC", math.floor(EI.energy(obj)),
                                    string.format("%.3f", EI.energyLevel(obj))}}
    end
end
...
function RCL.getTooltipInfos(obj, GUITable, mainFrame, justCreated)
    ...
    if justCreated == true then
        ...
        GAPI.addSimpleButton(GUITable, "R.C.L.OpenInvButton", inventoryFlow, {"gui-description.OpenInventory"}, "",
            false, {
                ID = obj.entID
            })
        ...
        GAPI.addLabel(GUITable, "", collectOresFlow, {"gui-description.RCLCollectOres"}, nil, "", false, nil,
            _mfLabelType.yellowTitle)
        -- Add the Collect Ores Switch --
        local state = obj.collectOres == true and "right" or "left"
        GAPI.addSwitch(GUITable, "R.C.L.CollectOres", collectOresFlow, {"gui-description.No"}, {"gui-description.Yes"},
            nil, nil, state, false, {
                ID = obj.ent.unit_number
            })
        ...
        GAPI.addLabel(GUITable, "", collectFluidFlow, {"gui-description.RCLCollectFluids"}, nil, "", false, nil,
            _mfLabelType.yellowTitle)
        -- Add the Collect Fluid Switch --
        state = obj.collectFluids == true and "right" or "left"
        GAPI.addSwitch(GUITable, "R.C.L.CollectFluids", collectFluidFlow, {"gui-description.No"},
            {"gui-description.Yes"}, nil, nil, state, false, {
                ID = obj.ent.unit_number
            })
        ...
        GAPI.addLabel(GUITable, "", settingsFrame, {"gui-description.RCLTargetedOresStorage"}, nil, "", false, nil,
            _mfLabelType.yellowTitle)
        ...
        for _, deepStorage in pairs(obj.dataNetwork.DSRTable) do
            ...
            if deepStorage ~= nil and deepStorage.ent ~= nil then
                ...
                if item ~= nil then
                    ...
                    table.insert(invs, {"", "[img=item/" .. itemName .. "] ", prototypes.item[itemName].localised_name, " - ",
                                        deepStorage.ID})
                else
        ...
        if selectedIndex ~= nil and selectedIndex > table_size(invs) then
            selectedIndex = nil
        end

        -- Add the Selected Deep Tank Drop Down --
        GAPI.addDropDown(GUITable, "R.C.L.TargetDSR", settingsFrame, invs, selectedIndex, false,
            {"gui-description.RCLOresInventorySelectTT"}, {
                ID = obj.ent.unit_number
            })
        ...
        -- Create the Select Storage Label --
        GAPI.addLabel(GUITable, "", settingsFrame, {"gui-description.RCLTargetedFluidStorage"}, nil, "", false, nil,
            _mfLabelType.yellowTitle)
        ...
        for _, deepTank in pairs(obj.dataNetwork.DTKTable) do
            ...
            if deepTank ~= nil and deepTank.ent ~= nil then
                ...
                -- Create the Name --
                if fluid ~= nil then
                    table.insert(invs, {"", "[img=fluid/" .. fluid .. "] ", prototypes.fluid[fluid].localised_name,
                                        " - ", deepTank.ID})
                else
        ...
        -- Add the Selected Deep Tank Drop Down --
        GAPI.addDropDown(GUITable, "R.C.L.TargetDTK", settingsFrame, invs, selectedIndex, false,
            {"gui-description.RCLFluidInventorySelectTT"}, {
                ID = obj.ent.unit_number
            })
    ...
    GAPI.addLabel(GUITable, "", informationTable, {"gui-description.QuatronCharge", Util.toRNumber(EI.energy(obj))},
        _mfOrange)
    GAPI.addProgressBar(GUITable, "", informationTable, "", EI.energy(obj) .. "/" .. EI.maxEnergy(obj), false,
        _mfPurple, EI.energy(obj) / EI.maxEnergy(obj), 100)

    -- Create the Quatron Purity --
    GAPI.addLabel(GUITable, "", informationTable,
        {"gui-description.Quatronlevel", string.format("%.3f", EI.energyLevel(obj))}, _mfOrange)
    GAPI.addProgressBar(GUITable, "", informationTable, "", "", false, _mfPurple, EI.energyLevel(obj) / 20, 100)

    -- Create the Speed --
    local speed = math.floor(
        (math.pow(EI.energyLevel(obj), _mfQuatronScalePower) + _mfOreCleanerResourcesPerExtraction) *
            EI.energyLevel(obj) * 3)
    ...
    GAPI.addLabel(GUITable, "", informationTable, {"", {"gui-description.RCLPathsCount"}, " [color=yellow]",
                                                   table_size(obj.resourcesTable), "[/color]"}, _mfOrange)
    ...
    GAPI.addLabel(GUITable, "", inventoryTable,
        {"gui-description.RCLTank1", Util.getLocFluidName(fluidsName1) or {"gui-description.Empty"}}, _mfOrange)
    GAPI.addProgressBar(GUITable, "", inventoryTable, nil, fluidsTT1, false, fluidsColor1,
        fluidsAmount1 / _mfRCLTankSize, nil)

    -- Create the Tank 2 Progress Bar --
    GAPI.addLabel(GUITable, "", inventoryTable,
        {"gui-description.RCLTank2", Util.getLocFluidName(fluidsName2) or {"gui-description.Empty"}}, _mfOrange)
    GAPI.addProgressBar(GUITable, "", inventoryTable, nil, fluidsTT2, false, fluidsColor2,
        fluidsAmount2 / _mfRCLTankSize, nil)

    -- Create the Tank 3 Progress Bar --
    GAPI.addLabel(GUITable, "", inventoryTable,
        {"gui-description.RCLTank3", Util.getLocFluidName(fluidsName3) or {"gui-description.Empty"}}, _mfOrange)
    GAPI.addProgressBar(GUITable, "", inventoryTable, nil, fluidsTT3, false, fluidsColor3,
        fluidsAmount3 / _mfRCLTankSize, nil)

    -- Create the Tank 4 Progress Bar --
    GAPI.addLabel(GUITable, "", inventoryTable,
        {"gui-description.RCLTank4", Util.getLocFluidName(fluidsName4) or {"gui-description.Empty"}}, _mfOrange)
    GAPI.addProgressBar(GUITable, "", inventoryTable, nil, fluidsTT4, false, fluidsColor4,
        fluidsAmount4 / _mfRCLTankSize, nil)
...
function RCL.update(obj)
    ...
    elseif obj.lightAn == nil or obj.lightAn.valid == false then
        obj.lightAn = rendering.draw_animation {
            animation = "ResourcesCollectorAn",
            target = obj.ent.position,
            surface = obj.ent.surface,
            render_layer = 144,
            animation_speed = 0.3
        }
    end
...
function RCL.scanResources(obj, entity)
    ...
    local area = {{entity.position.x - _mfRCLRadius, entity.position.y - _mfRCLRadius},
                  {entity.position.x + _mfRCLRadius, entity.position.y + _mfRCLRadius}}

    -- Check what have to be collected --
    if obj.collectOres == true and obj.collectFluids == true then
        -- Collect every Resources --
        obj.resourcesTable = entity.surface.find_entities_filtered {
            area = area,
            type = "resource",
            limit = EI.energyLevel(obj)
        }
...
function RCL.collectResources(obj)
    ...
    local toExtract = math.floor(math.pow(EI.energyLevel(obj), _mfQuatronScalePower) +
    ...
    for _, resourcesPath in pairs(obj.resourcesTable) do
        ...
        for _, product in pairs(storage.ResourcesProductsTable[resourcesPath.name]) do
            ...
            if product.type == "item" and obj.collectOres == true and math.random(0, 100) <= product.probability * 100 then
                ...
                inserted = inv.insert({
                    name = product.name,
                    count = amount * resourcesExtracted
                })
                -- If this is a Fluid --
            elseif product.type == "fluid" and obj.collectFluids == true and math.random(0, 100) <= product.probability *
                100 then
                -- Insert the Fluid inside the Tank --
                if obj.tank ~= nil then
                    inserted = obj.tank.insert_fluid({
                        name = product.name,
                        amount = (amount * resourcesExtracted) / 10
                    })
                    isFluid = true
                end
            end
            ...
            if inserted > 0 then
                ...
                obj.ent.surface.create_entity {
                    name = "RCLProjectile-" .. product.name,
                    position = resourcesPath.position,
                    target = obj.ent,
                    speed = 0.1,
                    max_range = 999,
                    force = obj.ent.force
...
function RCL.sendOreToDN(obj)
    ...
    local MFTooFar = true
    if obj.MF.ent ~= nil and obj.MF.ent.valid == true and obj.ent.surface == obj.MF.ent.surface and
        Util.distance(obj.ent.position, obj.MF.ent.position) <= _mfOreCleanerMaxDistance then
        MFTooFar = false
    end
    ...
    if MFTooFar == false and valid(obj.networkAccessPoint) and EI.energy(obj.networkAccessPoint) > 0 and
        Util.distance(obj.ent.position, obj.MF.ent.position) >
        Util.distance(obj.ent.position, obj.networkAccessPoint.ent.position) then
    ...
    if valid(obj.selectedInv) == true then
        ...
        local item = dsr.inventoryItem or dsr.filter
        local itemName = ""
        local itemQuality = "normal"
        local itemCount = 0

        if type(item) == "table" then
            itemName = item.name
            itemQuality = item.quality or "normal"
            itemCount = item.count or 0
        else
            itemName = item
            -- Check if the Resource Collector Inventory has this item --
            itemCount = inv.get_item_count(itemName)
        end
        ...
        local inserted = dsr:addItem(itemName, math.min(itemCount, dsr:availableSpace()), itemQuality)
    ...
    if obj.selectedInv == -1 then
        ...
        for _, item in pairs(inv.get_contents()) do
            ...
            for _, dsr in pairs(obj.dataNetwork.DSRTable) do
                ...
                local inserted = dsr:addItem(item.name, math.min(item.count, dsr:availableSpace()), item.quality)
                -- Remove the Items from the Inventory --
                if inserted > 0 then
                    inv.remove({
                        name = item.name,
                    ...
                    -- Create the Projectile --
                    RCL.createOreBeam(obj, item.name, target)
                end
                -- Decrease the count --
    ...
    if obj.selectedInv == -2 then
        ...
        for _, item in pairs(inv.get_contents()) do
            ...
            if item.count > 0 then
                local inserted = obj.dataNetwork:addItems(item, item.count)
                -- Remove the Items from the Inventory --
                if inserted > 0 then
                    inv.remove({
                        name = item.name,
...
function RCL.sendFluidToDN(obj)
    ...
    -- Check the Tank --
    if obj.tank == nil or obj.tank.valid == false then
        return
    end

    -- Stop if the selected Deep Tank is none --
    if obj.selectedDTK == nil then
        return
    end
    ...
    if valid(obj.selectedDTK) == true then
        ...
        local inserted = dtk:addFluid{
            name = fluidName,
            amount = math.min(fluidCount, dtk:availableSpace())
        }
        -- Remove the Items from the Inventory --
        if inserted > 0 then
            obj.tank.remove_fluid {
                name = fluidName,
                amount = inserted
            }
            -- Create the Projectile --
    ...
    if obj.selectedInv == -1 then
        ...
        for fluid, count in pairs(obj.tank.get_fluid_contents()) do
            ...
            for _, dtk in pairs(obj.dataNetwork.DTKTable) do
                ...
                local inserted = dtk:addFluid{
                    name = fluid,
                    amount = math.min(count, dtk:availableSpace())
                }
