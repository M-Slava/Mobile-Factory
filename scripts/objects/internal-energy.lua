...
function IEC:contentToItemTags(tags)
    ...
    tags.set_tag("Infos", {
        energy = self.ent.energy
    })
    tags.custom_description = {"", tags.prototype.localised_description,
                               {"item-description.EnergyCubeC", Util.toRNumber(math.floor(self.ent.energy))}}
end
...
function IEC:update()
    ...
    if (game.tick % 60 == 0) then
        local spriteNumber = math.ceil(self.ent.energy /
                                           self.ent.prototype.electric_energy_source_prototype.buffer_capacity * 16)
        if (self.sprite) then
            self.sprite.destroy()
        end
        self.sprite = rendering.draw_sprite {
            sprite = "CubeChargeSprite" .. spriteNumber,
            x_scale = 1 / 2.25,
            y_scale = 1 / 2.25,
            target = self.ent,
    ...
    EI.shareEnergy(self)
    self:shareEnergyWithEG()

end

function IEC:shareEnergyWithEG()
	if (self.MF == nil or self.MF.ent == nil or self.MF.ent.valid == false) then
		return
	end
	
    -- Get the Equipment Grid --
    local grid = self.MF.ent.grid
    if grid == nil then
        return
    end

    -- Get the Energy Equipments --
    local equipments = grid.equipment
    if equipments == nil then
        return
    end

    -- Loop on the Equipments --
    for _, equipment in pairs(equipments) do
        if equipment.type == "battery-equipment" and equipment.name == "mfInternalBatteryEquipment" then
            local balancedEnergy = (equipment.energy + self.ent.energy) / 2
            equipment.energy = balancedEnergy
            self.ent.energy = balancedEnergy
        end
    end
end
...
function IEC:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    GAPI.addProgressBar(GUITable, "", infoFrame, "", math.floor(EI.energy(self)) .. "/" .. EI.maxEnergy(self), false,
        _mfBlue, EI.energy(self) / EI.maxEnergy(self), 100)

    -- Add the Input/Output Speed Label --
    local inputLabel = GAPI.addLabel(GUITable, "", infoFrame,
        {"gui-description.EnergyInputSpeed", Util.toRNumber(EI.speed(self))}, _mfOrange)
    inputLabel.style.top_margin = 10
    GAPI.addLabel(GUITable, "", infoFrame, {"gui-description.EnergyOutputSpeed", Util.toRNumber(EI.speed(self))},
        _mfOrange)
