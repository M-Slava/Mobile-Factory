...
function DTK:contentToItemTags(tags)
    if self.inventoryFluid ~= nil or self.filter ~= nil then
        tags.set_tag("Infos", {
            inventoryFluid = self.inventoryFluid,
            inventoryCount = self.inventoryCount,
            filter = self.filter,
            inventoryTemperature = self.inventoryTemperature
        })
        tags.custom_description = {"", tags.prototype.localised_description,
                                   {"item-description.DeepTankC", self.inventoryFluid or self.filter,
                                    self.inventoryCount or 0, self.inventoryTemperature or 15}}
    end
end
...
function DTK:update()
    ...
    if self.inventoryFluid == nil and self.filter == nil then
        return
    end
...
function DTK:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    if justCreated == true then
        ...
        local filter = GAPI.addFilter(GUITable, "D.T.Filter", settingsFrame, nil, true, "fluid", 40, {
            ID = self.ent.unit_number
        })
        GUITable.vars.filter = filter

        -- Create the Max amout Textfield --
        GAPI.addLabel(GUITable, "", settingsFrame, {"gui-description.DSDTMaxFluidAmount"}, nil,
            {"gui-description.DSDTMaxFluidAmountTT"}, false, nil, _mfLabelType.yellowTitle)
        local textfield = GAPI.addTextField(GUITable, "D.T.Max", settingsFrame, self.max, "", false, true, false, false,
            false, {
                ID = self.ent.unit_number
            })
    ...
    GAPI.addLabel(GUITable, "", inventoryTable, {"gui-description.DSDTAmount2",
                                                 Util.toRNumber(self.inventoryCount or 0),
                                                 Util.toRNumber(self.max or _dtMaxFluid)}, _mfOrange)
...
function DTK:canAccept(fluid)
    ...
    if self.filter ~= nil and self.filter ~= fluid.name then
        return false
    end
    if self.inventoryFluid ~= nil and self.inventoryFluid ~= fluid.name then
        return false
    end
    if self.inventoryCount >= (self.max or _dtMaxFluid) then
        return false
    end
    if fluid.amount ~= nil and fluid.amount > (self.max or _dtMaxFluid) - self.inventoryCount then
        return false
    end
    return true
end
...
function DTK:addFluid(fluid)
    if self:canAccept(fluid) == true then
        ...
        self.inventoryTemperature =
            (self.inventoryTemperature * self.inventoryCount + added * (fluid.temperature or 15)) /
                (self.inventoryCount + added)
...
function DTK.interaction(event)
    ...
    if string.match(event.element.name, "D.T.Filter") then
        ...
        if storage.deepTankTable[id] == nil then
            return
        end
