...
function MI:new(object)
    ...
    t.stateSprite = rendering.draw_sprite {
        sprite = "MatterInteractorSprite1",
        target = object,
...
-- Is valid --
function MI:valid()
    if self.ent ~= nil and self.ent.valid then
        return true
    end
    return false
end
...
function MI:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    if justCreated == true then
        ...
        GAPI.addSimpleButton(GUITable, "M.I.OpenInvButton", inventoryFlow, {"gui-description.OpenInventory"}, "", false,
            {
        ...
        if valid(self.dataNetwork) == false then
            return
        end
        ...
        GAPI.addLabel(GUITable, "", parametersFrame, {"gui-description.MIFIChangeFilter"}, nil,
            {"gui-description.MIFIChangeFilterItemTT"}, false, nil, _mfLabelType.yellowTitle)
        local filter = GAPI.addFilter(GUITable, "M.I.Filter", parametersFrame, "", true, "item-with-quality", 40, {
            ID = self.ent.unit_number
        })
        ...
        GAPI.addLabel(GUITable, "", parametersFrame, {"gui-description.MIFIChangeMod"}, nil,
            {"gui-description.MIFIChangeModMITT"}, false, nil, _mfLabelType.yellowTitle)
        local state = "left"
        if self.selectedMode == "output" then
            state = "right"
        end
        GAPI.addSwitch(GUITable, "M.I.ModeSwitch", parametersFrame, {"gui-description.Input"},
            {"gui-description.Output"}, "", "", state, false, {
                ID = self.ent.unit_number
            })

        -- Prevent to store Item with Tags --
        if self.selectedMode == "input" and filter.elem_value ~= nil and prototypes.item[filter.elem_value.name].type ==
            "item-with-tags" then
            game.print("Inputing an item-with-tags ([item=" .. filter.elem_value.name ..
                           "]) erases tags (dangerous!). Clearing Matter Interactor values.")
        ...
        GAPI.addLabel(GUITable, "", parametersFrame, {"gui-description.MIFITargetedInv"}, nil,
            {"gui-description.MIFITargetedInvMITT"}, false, nil, _mfLabelType.yellowTitle)
        ...
        for k, deepStorage in pairs(self.dataNetwork.DSRTable) do
            if deepStorage ~= nil and deepStorage.ent ~= nil then
                ...
                elseif deepStorage.filter ~= nil then
                    item = deepStorage.filter.name
                end

                if item then
                    table.insert(invs, {"", "[img=item/" .. item .. "] ", prototypes.item[item].localised_name, " - ",
                                        deepStorage.ID})
                else
                    table.insert(invs, {"", "", {"gui-description.Empty"}, " - ", deepStorage.ID})
                end

                if self.selectedInv and type(self.selectedInv) == "table" and self.selectedInv.entID ==
                    deepStorage.entID then
                    selectedIndex = i
                end
            end
        end
        ...
        -- Create the DropDown --
        if selectedIndex > table_size(invs) then
            selectedIndex = nil
        end

        GAPI.addDropDown(GUITable, "M.I.TargetDD", parametersFrame, invs, selectedIndex, false, "", {
            ID = self.ent.unit_number
        })
    ...
    local filterName = self.selectedFilter ~= nil and Util.getLocItemName(self.selectedFilter) or
    ...
    if self.selectedFilter ~= nil then
        if type(self.selectedFilter) == "string" then
            GUITable.vars.filter.elem_value = {name=self.selectedFilter, quality="normal"}
        else
...
function MI:setActive(set)
    ...
    if set == true then
        ...
        self.stateSprite = rendering.draw_sprite {
            sprite = "MatterInteractorSprite2",
            target = self.ent,
...
function MI:updateInventory()
    ...
    if self.selectedMode == "input" then
        ...
        for _, item in pairs(inv.get_contents()) do
            ...
            local amountAdded = dataInv:addItem(name, count, quality)
    ...
    elseif self.selectedMode == "output" then
        ...
        local returnedItems = dataInv:hasItem(self.selectedFilter.name, self.selectedFilter.quality)
        -- Return if there are no Items --
        if returnedItems <= 0 then
            return
        end
        -- Insert requested Item inside the local Inventory --
        if returnedItems > 1000000 then
            returnedItems = 1000000
        end
        local addedItems = inv.insert({
            name = self.selectedFilter.name,
            count = returnedItems,
            quality = self.selectedFilter.quality
        })
        -- Remove Item from the Data Inventory --
        dataInv:getItem(self.selectedFilter.name, addedItems, self.selectedFilter.quality)
    end
...
function MI:blueprintTagsToSettings(tags)
    ...
    if tags["selectedMode"] then
        self.selectedMode = tags["selectedMode"]
    end
end
...
function MI.interaction(event, player)
    ...
    if string.match(event.element.name, "M.I.Filter") then
        local objId = event.element.tags.ID
        if storage.matterInteractorTable[objId] == nil then
            return
        end
