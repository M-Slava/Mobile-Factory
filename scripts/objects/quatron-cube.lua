...
function QC:contentToItemTags(tags)
    ...
    tags.set_tag("Infos", {
        ...
        purity = EI.energyLevel(self)
    })
    tags.custom_description = {"", tags.prototype.localised_description,
                               {"item-description.QuatronCubeC", math.floor(self.ent.energy),
                                string.format("%.3f", EI.energyLevel(self))}}
end
...
function QC:update()
    ...
    self.sprite = rendering.draw_sprite {
        ...
        x_scale = 1 / 7,
        y_scale = 1 / 7,
        target = {entity = self.ent, offset = {0, -0.3}},
...
function QC:getTooltipInfos(GUITable, mainFrame, justCreated)
    ...
    GAPI.addProgressBar(GUITable, "", infoFrame, "", EI.energy(self) .. "/" .. EI.maxEnergy(self), false, _mfPurple,
        EI.energy(self) / EI.maxEnergy(self), 100)

    -- Create the Quatron Purity --
    GAPI.addLabel(GUITable, "", infoFrame,
        {"gui-description.Quatronlevel", string.format("%.3f", EI.energyLevel(self))}, _mfOrange)
    ...
    local inputLabel = GAPI.addLabel(GUITable, "", infoFrame,
        {"gui-description.QuatronInputSpeed", Util.toRNumber(EI.speed(self))}, _mfOrange)
    inputLabel.style.top_margin = 10
    GAPI.addLabel(GUITable, "", infoFrame, {"gui-description.QuatronOutputSpeed", Util.toRNumber(EI.speed(self))},
        _mfOrange)
